...
const { parseCSVBuffer, readFileAsync } = require('./utils')
...
const loadRestaurantsCSVFile = async (filePath) => {
  ...
  await loadRestaurantsCSVData(data)
}

const loadRestaurantsCSVData = async (data) => {
  const restaurantsCSVData = await parseCSVBuffer(data)
...
const search = (searchDateTime) => {
  ...
  return candidates.filter(candidate => candidate.data.day === searchDay).map(candidate => candidate.data.name)
}

module.exports = {
  findOpenRestaurants,
  loadRestaurantsCSVData,
  search
}
